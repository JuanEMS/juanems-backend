...
function buildPDF(dataArray, columns, title, userInfo, dataCallback, endCallback) {
  ...
  drawHeaderLeftStyle(doc, title, userInfo);
  
  // Draw the table
  const tableEndY = drawTable(doc, dataArray, columns, colX, tableTop, maxY, title, userInfo);
...
function drawHeaderLeftStyle(doc, title, userInfo) {
  const marginLeft = doc.page.margins.left;
  const marginRight = doc.page.margins.right;
  const pageWidth = doc.page.width;
  const contentWidth = pageWidth - marginLeft - marginRight;
  ...
  if (userInfo && userInfo.userID && userInfo.fullName && userInfo.role) {
    doc.font('Helvetica')
      .fontSize(9)
      .fillColor(COLORS.darkGray)
      .text(`Generated by: ${userInfo.userID}-${userInfo.fullName}-${userInfo.role}`, 
        marginLeft, 
        headerHeight + 60, {
          align: 'right',
          width: contentWidth
...
// Updated continuation header to include user information
function drawContinuationHeader(doc, title, userInfo) {
  const marginLeft = doc.page.margins.left;
  const marginRight = doc.page.margins.right;
  const pageWidth = doc.page.width;
  const contentWidth = pageWidth - marginLeft - marginRight;
...
function drawTable(doc, dataArray, columns, colX, tableTop, maxY, title, userInfo) {
  let y = tableTop;
  ...
  for (let i = 0; i < dataArray.length; i++) {
    ...
    if (y + rowHeight > maxY) {
      doc.addPage();
      drawContinuationHeader(doc, title, userInfo);
